...
class ScreeningsImporter
  ...
  def get_the_previous
    @the_previous ||= {}.tap do |hsh|
      Screening.where(ticket_status: ['current', 'future']).select(:film_id, :cinema, :starts_at, :ticket_status, :sale_began_at).to_a.each do |screening|
        hsh[screening.identifier] = { ticket_status_was: screening.ticket_status, sale_began_at_was: screening.sale_began_at }
      end
    end
  end
